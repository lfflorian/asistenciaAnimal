// The file contents for the current environment will overwrite these during build.
// The build system defaults to the dev environment which uses `environment.ts`, but if you do
// `ng build --env=prod` then `environment.prod.ts` will be used instead.
// The list of which env maps to which file can be found in `.angular-cli.json`.

export const environment = {
  production: false,
  firebase: {
    apiKey: "AIzaSyB8WR4v_oG8pjjPXVl4y_Ml2ScdoU549Qw",
    authDomain: "asistenciaanimal-61b17.firebaseapp.com",
    databaseURL: "https://asistenciaanimal-61b17.firebaseio.com",
    projectId: "asistenciaanimal-61b17",
    storageBucket: "asistenciaanimal-61b17.appspot.com",
    messagingSenderId: "1011413892995",
    appId: "1:1011413892995:web:2d4cf28c66e479aeb980b6"
  }
};
